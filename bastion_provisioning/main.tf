...
resource "aws_subnet" "appsub" {
  ...
  count      = length(var.subnet_az)//creating two subnets
...
// Associating private subnet to the routetable 
resource "aws_route_table_association" "prvasort" {
  //count = 1
  subnet_id = aws_subnet.appsub[1].id
